...
`;
    ...
    /***/ './src/defaultResolver.ts': /***/ (
      ...
      const defaultAsyncResolver = (path, options) =>
    ...
    /***/ './src/index.ts': /***/ (
      ...
      var _exportNames = {};
      exports['default'] = void 0;
      var _resolver = _interopRequireDefault(
        __webpack_require__('./src/resolver.ts')
      );
    ...
    /***/ './src/resolver.ts': /***/ (
      ...
      var _ModuleNotFoundError = _interopRequireDefault(
        __webpack_require__('./src/ModuleNotFoundError.ts')
      );
      var _defaultResolver = _interopRequireWildcard(
        __webpack_require__('./src/defaultResolver.ts')
      );
      var _fileWalkers = __webpack_require__('./src/fileWalkers.ts');
      var _nodeModulesPaths = _interopRequireWildcard(
        __webpack_require__('./src/nodeModulesPaths.ts')
      );
      var _shouldLoadAsEsm = _interopRequireWildcard(
        __webpack_require__('./src/shouldLoadAsEsm.ts')
      );
      function _interopRequireDefault(e) {
        return e && e.__esModule ? e : { default: e };
      }
      ...
      class Resolver {
        ...
        constructor(moduleMap, options) {
          this._options = {
            ...
            hasCoreModules:
              options.hasCoreModules === undefined
                ? true
        ...
        async resolveModuleAsync(from, moduleName, options) {
          const dirname = path().dirname(from);
          const module =
            (await this.resolveStubModuleNameAsync(
              from,
              moduleName,
              options
            )) ||
        ...
        _prepareForResolution(dirname, moduleName, options) {
          ...
          const key =
            dirname + path().delimiter + moduleName + stringifiedOptions;
          const defaultPlatform = this._options.defaultPlatform;
          const extensions = [...this._options.extensions];
          if (this._supportsNativePlatform) {
            extensions.unshift(
              ...this._options.extensions.map(
                (ext) => `.${NATIVE_PLATFORM}${ext}`
              )
            );
          }
          if (defaultPlatform) {
        ...
        _throwModNotFoundError(from, moduleName) {
          const relativePath =
            (0, _slash().default)(
              path().relative(this._options.rootDir, from)
            ) || '.';
        ...
        isCoreModule(moduleName) {
          return (
            ...
            (0, _module().isBuiltin)(moduleName) &&
        ...
        getMockModule(from, name, options) {
          ...
          if (mock) {
            ...
            const resolvedName = this.resolveStubModuleName(
              from,
              name,
              options
            );
        ...
        async getModuleIDAsync(virtualMocks, from, moduleName = '', options) {
          ...
          const mockPath = await this._getMockPathAsync(
            from,
            moduleName,
            options
          );
        ...
        async _getVirtualMockPathAsync(
          virtualMocks,
          from,
          moduleName,
          options
        ...
        resolveStubModuleName(from, moduleName, options) {
          const dirname = path().dirname(from);
          const { extensions, moduleDirectory, paths } =
...
Please check your configuration for these entries:
{
  ...
  "resolver": ${_chalk().default.bold(String(resolver))}
    ...
    /***/ './src/shouldLoadAsEsm.ts': /***/ (
      ...
      const runtimeSupportsVmModules =
        typeof _vm().SyntheticModule === 'function';
      ...
      function shouldLoadAsEsm(path, extensionsToTreatAsEsm) {
        ...
        if (process.env.JEST_RESOLVE_NO_ESM) {
          return false;
        }
